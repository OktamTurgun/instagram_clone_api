...
class PostDetailView(RetrieveUpdateDestroyAPIView):
    ...
    queryset = Post.objects.select_related("user", "user__profile").prefetch_related(
        "images"
    )
    permission_classes = [IsAuthenticated]
...
class FeedView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Post.objects.filter(
                Q(user__in=following_users) | Q(user=user), is_archived=False
            )
            .select_related("user", "user__profile")
            .prefetch_related(
                "images",
                # 'likes' va 'comments'ni prefetch qilish ko'p xotira olishi mumkin
                # Agar serializerda faqat count ishlatilgan bo'lsa, bular shart emas.
            )
            .order_by("-created_at")
        )
...
class ExploreView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .exclude(Q(user__in=following_users) | Q(user=user))
            .select_related("user", "user__profile")
...
class UserPostsView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Post.objects.filter(user__username=username, is_archived=False)
            .select_related("user", "user__profile")  # QO'SHILDI: profil ma'lumotlari uchun
            .prefetch_related("images", "likes")
            .order_by("-created_at")
        )
...
class PostLikesListView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Like.objects.filter(post_id=post_id)
            .select_related("user", "user__profile") # QO'SHILDI: Avatar N+1 ni oldini oladi
            .order_by("-created_at")
        )
...
class CommentListCreateView(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .select_related("user", "user__profile") # QO'SHILDI: Asosiy comment muallifi
            .prefetch_related(
                "replies",
                "replies__user",
                "replies__user__profile", # QO'SHILDI: Reply yozganlarning avatarlari
            )
            .order_by("-created_at")
        )
...
class CommentDetailView(RetrieveUpdateDestroyAPIView):
    ...
    queryset = Comment.objects.select_related("user", "user__profile", "post") # profil qo'shildi
...
class SavedPostsListView(ListAPIView):
    ...
    def get_queryset(self):
        # Bu yerda select_related zanjiri (post__user__profile)
        # va prefetch_related ishlatilyapti
        return (
            SavedPost.objects.filter(user=self.request.user)
            .select_related(
                "post__user",
                "post__user__profile",  # Avatar N+1 muammosini oldini oladi
            )
...
class SavePostToggleView(APIView):
    ...
    def post(self, request, post_id):
        ...
        return Response(
            {"success": True, "message": message, "data": {"is_saved": is_saved}}
        )
